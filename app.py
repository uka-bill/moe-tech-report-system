...
SUPABASE_STORAGE_BUCKET = 'moe-images'
...
def upload_to_supabase_storage(file_data, filename, folder='reports'):
    ...
    try:
        ...
        # Get public URL
        public_url = supabase.storage.from_(SUPABASE_STORAGE_BUCKET).get_public_url(safe_filename)
        app.logger.info(f"✅ File uploaded to Supabase: {public_url}")
        return public_url
        
    except Exception as e:
        app.logger.warning(f"Supabase upload failed: {e}")
        return None
...
def delete_from_supabase_storage(file_url):
    ...
    try:
        ...
        if bucket_index > 0 and bucket_index < len(path_parts):
            ...
            if bucket == SUPABASE_STORAGE_BUCKET:
                supabase.storage.from_(SUPABASE_STORAGE_BUCKET).remove([file_path])
                app.logger.info(f"✅ Deleted from Supabase: {file_path}")
                return True
        
        return False
        
    except Exception as e:
        app.logger.warning(f"Supabase delete failed: {e}")
        return False

def init_supabase_storage_bucket():
    """Initialize the Supabase storage bucket if it doesn't exist"""
    if not supabase:
        return False
    
    try:
        # Try to create bucket (fails silently if exists)
...
def delete_mapping_image(image_id):
    """Delete a mapping image - removes from Supabase Storage if applicable"""
    try:
        if not supabase:
            return jsonify({'error': 'Database not connected'}), 500
        ...
        if image_response.data and image_response.data[0].get('image_url'):
            ...
            if 'supabase.co' in image_url:
                deleted = delete_from_supabase_storage(image_url)
                if deleted:
                    app.logger.info(f"✅ Deleted image from Supabase Storage")
                else:
                    app.logger.warning(f"Could not delete from Supabase Storage, may need manual cleanup")
            else:
                # Delete local file
    ...
    try:
        ...
        if supabase:
            try:
                ...
                supabase_url = upload_to_supabase_storage(file_content, filename, folder)
                if supabase_url:
...
def upload_mapping_image():
    """Upload a mapping image directly to Supabase Storage with metadata"""
    try:
        if not supabase:
            return jsonify({'error': 'Database not connected'}), 500
        ...
        if supabase:
            try:
                init_supabase_storage_bucket()
                supabase_url = upload_to_supabase_storage(file_content, filename, 'mapping')
                if supabase_url:
                    image_url = supabase_url
                    uploaded_to_supabase = True
                    app.logger.info(f"✅ Mapping image uploaded to Supabase: {image_url}")
            except Exception as e:
        ...
        if not image_url:
            ...
            with open(filepath, 'wb') as f:
                f.write(file_content)
            image_url = f"/api/images/{filename}"
            app.logger.info(f"📁 Mapping image saved locally: {filepath}")
        ...
        if result.data:
            app.logger.info(f"✅ Mapping image record created: ID {result.data[0]['id']}")
            return jsonify({
                'success': True,
                'image': result.data[0],
                'image_url': image_url,
        ...
        app.logger.error(f"Error uploading mapping image: {e}")
        return jsonify({'error': str(e)}), 500
